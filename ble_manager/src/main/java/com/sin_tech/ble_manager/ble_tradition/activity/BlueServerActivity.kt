...
class BlueServerActivity : AppCompatActivity(), ServiceConnection, ClientCallback,
    ...
    private var service: BlueServerService? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //绑定服务
        bindService(
            Intent(this, BlueServerService::class.java),
            this,
            BIND_AUTO_CREATE
        )
...
//        startActivityForResult(Intent.createChooser(intent, "选择媒体"), 1001)
        ...
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { it2 ->
            if (it2.resultCode == Activity.RESULT_OK) {
                ...
                photoUri?.let { uri ->
                    ...
                    val filePath = UriToPathUtil.getPathFromUri(this@BlueServerActivity,uri)
                    filePath?.let { path ->
    ...
    fun toast(str:String){
        runOnUiThread {
            Toast.makeText(this@BlueServerActivity,str,Toast.LENGTH_SHORT).show()
        }
    }
...
//        WiFiDirectJobService.scheduleJob(this)
    ...
    private fun stopWiFiDirectService() {
        // 停止前台服务
        BlueServerService.stopService(this)
        // 取消Job
