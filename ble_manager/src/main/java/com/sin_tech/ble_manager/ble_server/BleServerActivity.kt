...
import com.sin_tech.ble_manager.ble_tradition.service.BlueServerService
import com.sin_tech.ble_manager.databinding.BlueServerLayoutBinding
...
class BleServerActivity : AppCompatActivity(), ServiceConnection, ServerCallback,
    ServerFileTransferCallback{
    private val REQUEST_DISCOVERABLE: Int = 101
    private val STORAGE_PERMISSION_REQUEST_CODE: Int = 102

    private var binding: BlueServerLayoutBinding? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = BlueServerLayoutBinding.inflate(layoutInflater)
...
//        startActivityForResult(Intent.createChooser(intent, "选择媒体"), 1001)
        ...
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { it2 ->
            if (it2.resultCode == RESULT_OK) {
                ...
                photoUri?.let { uri ->
                    ...
                    val filePath = UriToPathUtil.getPathFromUri(this@BleServerActivity,uri)
                    filePath?.let { path ->
    ...
    fun toast(str:String){
        runOnUiThread {
            Toast.makeText(this@BleServerActivity,str,Toast.LENGTH_SHORT).show()
        }
    }
...
//                FileUtils.moveFile(filePath, destFile)
    ...
    private fun startBlueService() {
        // 启动前台服务
        BlueServerService.startService(this)
        // 调度Job（Android 5.0+）
//        WiFiDirectJobService.scheduleJob(this)
        // 更新UI
        isServiceRunning = true
        Toast.makeText(this, "Blue 服务已启动", Toast.LENGTH_SHORT).show()
    }

    /**
     * 停止WiFi Direct服务
     */
    private fun stopBlueService() {
        // 停止前台服务
...
//        WiFiDirectJobService.cancelJob(this)
    ...
    override fun onServiceConnected(name: ComponentName?, binder: IBinder) {
        service = (binder as BleForegroundService.SerialBinder).service
        service?.setWeakRef(WeakReference(this@BleServerActivity))
        makeDiscoverable()
    }
