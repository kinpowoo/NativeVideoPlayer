...
//                Log.d("Explorer", "Header: " + header.toString());
        ...
        private void sendResponse(Socket socket, String status, String mime, Properties header, StreamSource data) {
            ...
            try {
                ...
                bos = new BufferedOutputStream(out, 64 * 1024);  // 64KB缓冲区
//                OutputStream out = socket.getOutputStream();
                ...
                if (data != null) {
                    ...
                    data.open();
                    // 使用更大的读取缓冲区
                    byte[] buff = new byte[128 * 1024];  // 128KB读取缓冲区
                    int read = 0;
                    long totalRead = 0;
...
//                        out.write(buff, 0, read);
                        ...
                        // 每读取一定量刷新一次，避免缓冲区过大
                        if (totalRead % (1024 * 1024) == 0) {  // 每1MB刷新一次
                            bos.flush();
                        }
                    }
                }
    ...
    private String encodeUri(String uri) {
        ...
        while (st.hasMoreTokens()) {
            ...
            if (tok.equals("/")) {
                ...
                newUri.append(URLEncoder.encode(tok));
